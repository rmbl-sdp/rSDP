...
writeLines(snow_persist_meta$qgis$abstract[[1]][1])
```

We can also plot this dataset on a web map:

```{r plet snow,out.width="100%"}
plet(snow_persist_mean,tiles="Streets",main="Snowpack Persistence \n (day of year)")
```
...
```{r snow multilayer}
cat_snow_yearly <- sdp_get_catalog(types=c("Snow"),releases=c("Release4"),timeseries_type="Yearly")
...
```{r extract simple}
snow_sample_simple <- sdp_extract_data(snow_persist_recent,sites_proj,method="simple")
...
```{r extract compare}
snow_sample <- as.data.frame(rbind(snow_sample_simple, snow_sample_bilin))
snow_long <- tidyr::pivot_longer(snow_sample,cols=contains("X"),names_to="year",values_to="snow_persist_doy")
snow_wide <- tidyr::pivot_wider(snow_long,names_from="method",values_from="snow_persist_doy")

plot(bilinear~simple,data=snow_wide,xlab="Simple Extraction",ylab="Bilinear Interpolation")
```
...
plet(vect(sites_buff100),tiles="Streets",col="grey20",alpha=0.2) |>
...
```{r buff ext}
snow_sample_mean <- sdp_extract_data(snow_persist_recent,vect(sites_buff100))
snow_sample_mean$stat <- "mean"
snow_sample_min <- sdp_extract_data(snow_persist_recent,vect(sites_buff100),
                                    sum_fun="min")
snow_sample_min$stat <- "min"
snow_sample_q25 <- sdp_extract_data(snow_persist_recent,vect(sites_buff100),
                                    sum_fun=function(x) quantile(x,probs=c(0.25))) #25th percentile
snow_sample_q25$stat <- "q25"
snow_sample_q75 <- sdp_extract_data(snow_persist_recent,vect(sites_buff100),
                                    sum_fun=function(x) quantile(x,probs=c(0.75))) # 75th percentile
snow_sample_q75$stat <- "q75"
snow_sample_max <- sdp_extract_data(snow_persist_recent,vect(sites_buff100),
                                    sum_fun="max")
...
snow_sample_stats <- as.data.frame(rbind(snow_sample_min,
                           ...
                           snow_sample_q75,
                           snow_sample_max))
snow_stats_long <- tidyr::pivot_longer(snow_sample_stats,cols=contains("X"),
                                names_to="year",values_to="snow_persist_doy")
snow_stats_wide <- tidyr::pivot_wider(snow_stats_long,names_from="stat",values_from="snow_persist_doy")
...
snow_gt_q10 <- sdp_extract_data(snow_persist_recent,gt_poly,
                                    sum_fun=function(x) quantile(x,probs=c(0.1))) #25th percentile
snow_gt_q10$stat <- "q10"
snow_gt_q90 <- sdp_extract_data(snow_persist_recent,gt_poly,
                                    sum_fun=function(x) quantile(x,probs=c(0.9))) # 75th percentile
...
days <- seq(as.Date("2018-06-01"),as.Date("2020-05-30"), by="month")

extr_list <- foreach(i=1:length(days),.packages=c("terra","rSDP","sf")) %dopar% {
  tmax <- sdp_get_raster("R4D007",date_start=days[i],date_end=days[i],verbose=FALSE)
   locations_sv <- vect(locations_sf)
   extr_dat <- sdp_extract_data(tmax,locations=locations_sv,verbose=FALSE,
                                      return_type="SpatVector")[,4]
   extr_df <- as.data.frame(st_drop_geometry(st_as_sf(extr_dat)))
   (extr_df)
}
stopCluster(cl)
tmax_extr <- cbind(locations_sf,do.call(cbind,extr_list))
